...
function createSetting(&$modx,$key,$value) {
    $ct = $modx->getCount('modSystemSetting',array(
        'key' => 'wxgotowebinar.'.$key,
    ));
    if (empty($ct)) {
        $setting = $modx->newObject('modSystemSetting');
        $setting->set('key','wxgotowebinar.'.$key);
        $setting->set('value',$value);
        $setting->set('namespace','wxgotowebinar');
...
if ($object->xpdo) {
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            ...
            /* setup paths */
            createSetting($modx,'core_path',$modx->getOption('core_path').'components/wxgotowebinar/');
            createSetting($modx,'assets_path',$modx->getOption('assets_path').'components/wxgotowebinar/');

            /* setup urls */
            createSetting($modx,'assets_url',$modx->getOption('assets_url').'components/wxgotowebinar/');
        break;
    }
}
return true;